...
import { ChevronLeft, ChevronRight, Maximize2, Bed, ArrowRight } from "lucide-react";

export default function ImperialRooms() {
  const rooms = getFeaturedRooms();
  const [index, setIndex] = useState(0);

  const next = useCallback(() => setIndex((prev) => (prev + 1) % rooms.length), [rooms.length]);
  const prev = useCallback(() => setIndex((prev) => (prev - 1 + rooms.length) % rooms.length), [rooms.length]);
    ...
    <section className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-32 overflow-hidden">
      <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
        <div className="max-w-2xl">
          ...
          >
            Each suite is a masterpiece of design, where hand-carved granite meets the finest Mysore silks, recreating the living standards of Chola emperors.
          </motion.p>
        </div>
      ...
      <div className="relative cursor-grab active:cursor-grabbing">
        <div className="overflow-visible">
          <AnimatePresence mode="wait">
            ...
            >
              {[0, 1, 2].map((offset) => {
                const roomIndex = (index + offset) % rooms.length;
                  ...
                  >
                    <div className="h-[300px] md:h-[400px] overflow-hidden relative">
                      ...
                      <div className="absolute bottom-6 left-6 text-on-primary translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                        <span className="font-label-caps text-[10px] tracking-widest mb-1 block">STARTING FROM</span>
                        <div className="text-2xl font-headline-md text-white">₹{room.price.toLocaleString('en-IN')} <span className="text-sm font-body-md opacity-70">/ NIGHT</span></div>
                      </div>
                    </div>
                    <div className="p-8 flex flex-col flex-grow border-t border-outline-variant/10">
                      <h3 className="font-headline-md text-2xl mb-4 text-on-surface group-hover:text-royal-gold transition-colors">{room.name}</h3>
